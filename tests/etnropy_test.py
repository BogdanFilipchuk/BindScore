...
def debug_entropy(result: Protein_Entropy, protein_id: str, mode: str) -> None:
    if DEBUG_OVERAL_ENTROPY:
        print(f'[OVERALL]      {protein_id} ({mode}): {result.dS_total:.4f} J/(mol·K)')
    if DEBUG_TRANS_ROT_ENTROPY:
        print(f'[TRANS_ROT]    {protein_id} ({mode}): {result.dS_trans_rot:.4f} J/(mol·K)')
    if DEBUG_HYDROPHOBIC_ENTROPY:
        print(f'[HYDROPHOBIC]  {protein_id} ({mode}): {result.dS_hydrophobic:.4f} J/(mol·K)')
    if DEBUG_SIDECHAIN_ENTROPY:
        print(f'[SIDECHAIN]    {protein_id} ({mode}): {result.dS_sidechain:.4f} J/(mol·K)')
    if DEBUG_BACKBONE_ENTROPY:
        print(f'[BACKBONE]     {protein_id} ({mode}): {result.dS_backbone:.4f} J/(mol·K)')
...
total_entropy_result = compute_total_entropy(test_pdb_path,"A","B", T=T_KELVIN, HYDROPHOBIC_SETTING="SASA", return_breakdown=True)
debug_entropy(total_entropy_result, myprotein.get_ID(), "SASA")

print()

"Calculating the entropy result"
total_entropy_result = compute_total_entropy(test_pdb_path,"A","D", T=T_KELVIN, HYDROPHOBIC_SETTING="Sun", return_breakdown=True)
