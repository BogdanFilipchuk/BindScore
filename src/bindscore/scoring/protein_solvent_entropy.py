...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Calculate solute–solvent entropy for a protein in water.\n"
            "The protein is read from a PDB file and treated as a sphere.\n"
            "Water constants are fixed from Sun (2022) Molecules 27, 7009."
        ),
    ...
    parser.add_argument(
        "pdb",
        nargs="?",
        default=Path("tests/test_pdb_files/6PYH.pdb"),
        help="Path to the PDB file (.pdb)"
    )
    parser.add_argument(
        "--temp", "-t",
        type=float,
        default=293.0,
        metavar="K",
        help="Temperature in Kelvin (default: 293.0 K)"
    )
    parser.add_argument(
        "--chain", "-c",
        default=None,
        metavar="ID",
        help="Only use residues from this chain (e.g. A). Default: all chains."
    )

    args = parser.parse_args()

    name, sequence, radius = parse_protein(args.pdb, args.chain)
    print_results(name, sequence, radius, args.temp)
